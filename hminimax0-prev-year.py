...
def distance_to_fruit(state):
    ...
    fruits = PriorityQueue()
    pos1 = state.getPacmanPosition()
    for i in range(food_grid.width):
        for j in range(food_grid.height):
            if food_grid[i][j]:
                fruits.push((i, j), manhattanDistance(pos1, (i, j)))

    _, furthest_fruit = fruits.pop()
    if fruits.isEmpty():
        path, _ = astar(state, furthest_fruit)  # Real distance between pacman and the remaining fruit
        return len(path)
    else:
        _, second_furthest_fruit = fruits.pop()

        path1, result_state = astar(state, second_furthest_fruit)   # Real distance between
        # pacman and second furthest fruit

        path2, _ = astar(result_state, furthest_fruit)  # Real distance between
        # the two furthest fruits
...
def generate_tree(current_state, depth, initial_fruit_count, visited):
    ...
    if cut_off(current_state, depth) is True or terminal(current_state) == 1:
        current_node.set_minimax_value(eval(current_state, 1, depth, fruit_point))
        return current_node

    begin_successors = current_state.generatePacmanSuccessors() if player_turn == 1 \
        else current_state.generateGhostSuccessors(1)
